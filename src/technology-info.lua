...
local function generate_all_techs()
    local technologies = {}
    ...
    local function no_tech_factory() return 0, 0, 0 end

    for index, qol_type in ipairs(qol_types) do
        if qol_type.bonus_fields then
            local tiers
            if settings.startup[('qol-%s-research-enabled'):format(qol_type.name)].value then
                ...
                tiers = parse_config(config, qol_type.allow_rational_bonus)
            else
                print(('research for %s is not enabled, setting tiers to empty array'):format(qol_type.name))
                tiers = {}
            end
...
local lazy_techs
return function ()
    if not lazy_techs then
        lazy_techs = generate_all_techs()
    end
    return lazy_techs
end
